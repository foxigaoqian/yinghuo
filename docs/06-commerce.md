...
报价10分钟有效（配置建议）；修改素材、动作、商品或TA后旧报价失效。创建订单事务消费quote并预留抵扣，pending订单15分钟到期（建议），到期先向渠道核对/关单再释放抵扣。仅本地计时超时不能断言未收款。

第二位TA先draft；年费购买成功再激活。若已占免费名额且只想做独立影像，允许按普通价购买而不建立新空间。已有有效记忆会员购买声音用upgrade，不重复收两个完整年费。

## 3. 微信等支付通道接入要求

分别判断普通H5、微信内网页与小程序的可用支付方式、商户主体及appId绑定；不要用一个web跳转方案覆盖所有端。接入时以官方当前SDK/文档为准，本文不硬编码平台字段或审核结论。

支付回调使用原始请求体验签，按所用协议解密，校验证书/公钥、事件标识、时间、商户与应用身份；核对订单号、币种、金额和渠道交易号。拒绝验签失败或金额不符的请求，不能仅依据payload里的success。

确认事务：锁Order → 去重channel/event及tradeNo → 校验金额 → 写Payment succeeded → Order paid → 写Outbox → 提交 → 回调确认。发送任务与开权益不能发生在数据库提交之前。

同一订单多渠道尝试若出现重复实收，不能开两份权益；记录重复交易，停止二次交付并进入重复付款退款处理。回调丢失时定时主动查单，对账检查“渠道已付而本地待付”“本地已付但未交付”“退款未确定”。
...
重复回调10次只开一次；支付成功前端关闭仍交付；查单成功与回调同时到达不重复；两个订单争同一抵扣只成功一份；退款接口超时后能查回；金额被篡改拒绝；套餐过期边界正确；支付晚到不吞款；声音激活失败不损失旧年费；容量满仍可下载作品和导出。
